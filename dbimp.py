...
def main():
    ...
    ls = os.listdir('F:\DjangoWork\InCT_Web_Site\collected_static\images\math')
    i = 1
    for item in ls :
        ls_a = os.listdir(item)
        for image in ls_a:
            ClassImage.objects.create(classID=2, className='xd', sectionID=int(item.split('_')[4]), imageSeq=i, imagePath=image)
            i += 1
